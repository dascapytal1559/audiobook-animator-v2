 ...
 * to the next chunk's start; only when even that is narrower than the text is it clipped with an ellipsis (the title carries the full
 * text). In the Edited row, selected boxes are highlighted and a word carries marks for its manual and auto layers (CSS only).
 */
export function ChunkLane({ variant, mode, chunks, words, viewStartSample, viewEndSample, pxPerSample, currentWordId, selectedIds, onWordClick, onChunkClick, onItemPointerDown }: Props) {
  ...
  if (mode === "words") {
    return <WordBoxes variant={variant} words={words} viewStartSample={viewStartSample} viewEndSample={viewEndSample} pxPerSample={pxPerSample} currentWordId={currentWordId} selectedIds={selectedIds} onWordClick={onWordClick} {...(onItemPointerDown !== undefined ? { onItemPointerDown } : {})} />;
  }
  ...
  return (
    <div className={`lane lane-chunks row-${variant}`} data-row={variant}>
      ...
      {visible.map(({ chunk, next }) => {
        ...
        const maxWidth = next === undefined ? undefined : Math.max(minWidth, (next.startSample - chunk.startSample) * pxPerSample - CHUNK_GAP_PX);
...
function WordBoxes({ variant, words, viewStartSample, viewEndSample, pxPerSample, currentWordId, selectedIds, onWordClick, onItemPointerDown }: WordProps) {
  const interactive = onItemPointerDown !== undefined;
  const margin = (viewEndSample - viewStartSample) / 2;
  ...
  return (
    <div className={`lane lane-chunks lane-words row-${variant}`} data-row={variant}>
      ...
      {visible.map(({ word, next }) => {
        ...
        const maxWidth = next === undefined ? undefined : Math.max(minWidth, (next.startSample - word.startSample) * pxPerSample - CHUNK_GAP_PX);
        return (
          <div
