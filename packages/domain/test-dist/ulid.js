...
export function encodeUlid(timestamp, randomness) {
    if (!Number.isSafeInteger(timestamp) || timestamp < 0 || timestamp >= 2 ** 48)
        throw new RangeError("ULID timestamp must be an integer of at most 48 bits.");
    if (randomness < 0n || randomness >= 1n << 80n)
