...
export const SHOT_MODES = ShotMode.literals;
...
export const DEFAULT_SETTINGS = { frameAspect: { width: 16, height: 9 } };
 ...
 */
export function mergeTimeline(records, decisions, sampleCount, wordStarts) {
    const byId = new Map(records.map(r => [r.id, r]));
    const problems = [];
    const unresolvedAnchors = [];
    for (const [id, decision] of Object.entries(decisions.shots)) {
        if (!byId.has(id))
            problems.push(`Decision references a shot with no generation record: ${id}`);
        if (decision.startSample !== undefined && decision.startSample >= sampleCount)
            problems.push(`Decision startSample ${decision.startSample} is outside the clip's ${sampleCount} samples for shot ${id}`);
        if (decision.selected === true && decision.hidden === true)
            problems.push(`Shot ${id} is both selected and hidden`);
        if (decision.anchorWordId !== undefined && !wordStarts.has(decision.anchorWordId)) {
            if (wordStarts.size > 0)
                problems.push(`Shot ${id} is anchored to a word that is not in the transcript: ${decision.anchorWordId}`);
            else
    ...
    for (const record of records) {
        ...
        const shot = { ...fields, startSample: anchored ?? d.startSample ?? record.startSample, ...(d.anchorWordId !== undefined ? { anchorWordId: d.anchorWordId } : {}),
            ...
            hidden: d.hidden === true, selected: false, ...(d.selected === true ? { selectionSource: "decision" } : {}) };
    ...
    for (const startSample of [...groups.keys()].sort((a, b) => a - b)) {
        ...
        if (explicit.length > 1)
            problems.push(`More than one shot selected at sample ${startSample}: ${explicit.map(s => s.id).join(", ")}`);
        const visible = shots.filter(s => !s.hidden);
        const chosen = explicit[0] ?? visible[0];
        for (const s of shots) {
            if (s !== chosen)
                delete s.selectionSource;
        }
        if (chosen) {
            chosen.selected = true;
            chosen.selectionSource = explicit[0] ? "decision" : "default";
        }
    ...
    const stitched = selected.map((shot, i) => ({ kind: "shot", ...shot, endSample: selected[i + 1]?.startSample ?? sampleCount }));
    const firstStart = stitched[0]?.startSample ?? sampleCount;
    if (firstStart > 0)
        stitched.unshift({ kind: "gap", startSample: 0, endSample: firstStart });
...
export function entryAt(stitched, sample, toleranceSamples) {
    let current = null;
    for (const entry of stitched) {
        if (entry.startSample <= sample + toleranceSamples)
            current = entry;
        else
            break;
    }
