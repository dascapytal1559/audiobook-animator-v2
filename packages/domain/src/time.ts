...
export function samplesToSeconds(sample: number, sampleRateHz: number): number {
  assertRate(sampleRateHz);
  return sample / sampleRateHz;
}

/** Rounds to the nearest integer sample and clamps at zero. */
export function secondsToSamples(seconds: number, sampleRateHz: number): number {
  assertRate(sampleRateHz);
...
export function millisecondsToSamples(milliseconds: number, sampleRateHz: number): number {
...
export function formatSampleClock(sample: number, sampleRateHz: number): string {
...
export function clampSample(sample: number, sampleCount: number): number {
  if (!Number.isInteger(sampleCount) || sampleCount < 1) throw new RangeError(`sampleCount must be a positive integer, got ${sampleCount}.`);
  return Math.min(sampleCount - 1, Math.max(0, Math.round(sample)));
}

function assertRate(sampleRateHz: number): void {
  if (!Number.isInteger(sampleRateHz) || sampleRateHz < 1) throw new RangeError(`sampleRateHz must be a positive integer, got ${sampleRateHz}.`);
}
