...
export class IoError extends Data.TaggedError("IoError")<{
  readonly code: "IoFailed" | "InvalidJson" | "SchemaMismatch";
  readonly message: string;
  readonly path: string;
}> {}
const fail = (code: IoError["code"], path: string, message: string) => Effect.fail(new IoError({ code, path, message }));

/** Read a regular file of at most `limit` bytes on one handle, failing if it grows, shrinks, or is touched while being read. */
export function readBounded(path: string, limit: number): Effect.Effect<Buffer, IoError, FileSystem.FileSystem> {
  ...
  })).pipe(Effect.mapError(error => error instanceof IoError ? error : new IoError({ code: "IoFailed", path, message: `Cannot read input: ${path}.` })));
}
...
export function decodeJson<S extends Schema.Top>(schema: S, bytes: Uint8Array, path: string, strict: boolean): Effect.Effect<S["Type"], IoError, S["DecodingServices"]> {
  return Effect.gen(function* () {
    const raw = yield* Effect.try({
      try: () => JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes)) as unknown,
      catch: () => new IoError({ code: "InvalidJson", path, message: `Input is not valid UTF-8 JSON: ${path}.` }),
    });
    return yield* Schema.decodeUnknownEffect(schema, { onExcessProperty: strict ? "error" : "ignore" })(raw).pipe(
      Effect.mapError(e => new IoError({ code: "SchemaMismatch", path, message: `Input does not match the required schema: ${path}. ${e.message.replace(/\s+/g, " ")}` })),
    );
  });
}
...
export function writeAtomic(path: string, bytes: Uint8Array): Effect.Effect<void, IoError, FileSystem.FileSystem> {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
  ...
  }).pipe(Effect.mapError(e => new IoError({ code: "IoFailed", path, message: `Cannot write ${path}: ${e.message}` })));
}
