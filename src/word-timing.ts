...
const usage = `Usage:
  node dist/word-timing.js align   --story <id> [--run <file>] (--from S --to S | --all) [--dry-run]
  node dist/word-timing.js measure --story <id> [--run <file>] --from S --to S
...
const main = Effect.gen(function* () {
  const { values, positionals } = yield* Effect.try({
    try: () => parseArgs({ options: { run: { type: "string" }, story: { type: "string" }, help: { type: "boolean" }, from: { type: "string" }, to: { type: "string" }, all: { type: "boolean" }, "dry-run": { type: "boolean" } }, strict: true, allowPositionals: true }),
  ...
  const storyId = values.story === undefined || values.story.trim() === "" ? undefined : values.story;
  if (storyId === undefined) { yield* selectStory(run, undefined); return; }
  const ctx = yield* loadEditorContext({ storiesDirectory: run.storiesDirectory, settings: { story: run.story, timeline: run.timeline, editor: run.editor }, storyId });
  ...
  yield* print({ range, regionCount: speech.regions.filter(r => r.endSample > range.startSample && r.startSample < range.endSample).length, boundaryPauseMs: ctx.settings.editor.alignment.boundaryPauseMs,
...
main.pipe(
  Effect.catch(error => Console.error(error instanceof EditorServerError || error instanceof StoryError || error instanceof WordTimingError || error instanceof RunError ? `${error.code}: ${error.message}` : `Cannot run word timing: ${String(error)}`).pipe(
