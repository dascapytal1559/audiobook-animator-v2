...
import { addShot, DEFAULT_SETTINGS, loadVisualTimeline, ShotMode, type ShotRecord, timelineError, writeDecisions } from "../modules/visual-timeline/index.js";
...
const tool = (file: string, args: ReadonlyArray<string>) => Effect.tryPromise({
  ...
  catch: e => timelineError({ code: "IoFailed", message: `${file} ${args.join(" ")} failed: ${e instanceof Error ? e.message : String(e)}` }),
});
...
const seed = Command.make("seed", {
  ...
  const temp = yield* Effect.tryPromise({ try: () => mkdtemp(join(tmpdir(), "visual-timeline-seed-")), catch: () => timelineError({ code: "IoFailed", message: "Cannot create a temporary directory." }) });
  const work = Effect.gen(function* () {
    ...
    for (const s of seeds) {
      ...
      if (probe.stdout.trim() !== `${crop.width},${crop.height}`) return yield* Effect.fail(timelineError({ code: "IoFailed", message: `Cropped panel ${s.key} is ${probe.stdout.trim()}, expected ${crop.width},${crop.height}.` }));
      panels.set(s.id, { path: out, sha256: sha256(yield* fs.readFile(out).pipe(Effect.mapError(() => timelineError({ code: "IoFailed", message: `Cannot read ${out}.` })))) });
    }
    ...
    for (const s of seeds) {
      ...
      if (s.image && existing.imagePath !== undefined) {
        ...
        const bytes = yield* fs.readFile(imagePath).pipe(Effect.mapError(() => timelineError({ code: "IoFailed", message: `Cannot read ${imagePath}.` })));
    ...
    if (mismatches.length > 0) return yield* Effect.fail(timelineError({ code: "RecordExists", message: `Existing records differ from the seed; nothing was written. Differing fields:\n  ${mismatches.join("\n  ")}` }));
    ...
    const decisionsExist = yield* fs.exists(decisionsPath).pipe(Effect.mapError(() => timelineError({ code: "IoFailed", message: `Cannot inspect ${decisionsPath}.` })));
