...
test("status reports facts for a verified story: no overlays, no records, absent caches, its documents, and a server that is not listening", async t => {
  ...
  assert.deepEqual(s.timeline, { records: 0, withImage: 0, selected: 0, hidden: 0, gaps: [{ startSample: 0, endSample: 100 }], undecidedGroups: 0, unresolvedAnchors: [] });
...
test("status counts records, selections, undecided groups, an auto overlay's coverage and parameter drift, and reports a broken story instead of omitting it", async t => {
  ...
  const record = async (startSample: number, createdAt: string) => {
    const id = mintUlid();
    await mkdir(join(f.dir, "shots", id), { recursive: true });
    await writeFile(join(f.dir, "shots", id, "record.json"), encode({ schemaVersion: 1, kind: "visual-shot-generation", id, clip, startSample, mode: "graphic-illustration", createdAt, producer: { name: "t", version: "1" } }));
  };
  ...
  assert.ok(s.verified);
  if (!s.verified) return;
  assert.deepEqual(s.timeline, { records: 3, withImage: 0, selected: 2, hidden: 0, gaps: [], undecidedGroups: 1, unresolvedAnchors: [] });
