...
import { computePeaksAndSpeech, isEditorError, PeakAccumulator, type PeaksIdentity, readPeaksCache, readSpeechCache, type SpeechIdentity, writeCache } from "./index.js";
...
test("computePeaksAndSpeech decodes a real FLAC through ffmpeg once, yields peaks and speech regions, and refuses a sample count that differs from the verified clip", async t => {
  ...
  const wrongCount = await compute({ identity: { sampleCount: 8001 }, speech: { sampleCount: 8001 } }).then(() => null, e => e as AnimatorError);
  assert.ok(isEditorError(wrongCount) && wrongCount.code === "PeaksFailed" && /decoded 8000 samples but the verified clip has 8001/.test(wrongCount.message));
  const mismatch = await compute({ speech: { audioSha256: "d".repeat(64) } }).then(() => null, e => e as AnimatorError);
  assert.ok(isEditorError(mismatch) && /different clips/.test(mismatch.message));
  await writeFile(audioPath, "not audio");
  const garbage = await compute().then(() => null, e => e as AnimatorError);
  assert.ok(isEditorError(garbage) && garbage.code === "PeaksFailed" && /exited with code/.test(garbage.message), garbage?.message);
  const missing = await compute({ ffmpegPath: join(dir, "no-such-ffmpeg") }).then(() => null, e => e as AnimatorError);
  assert.ok(isEditorError(missing) && missing.code === "PeaksFailed", missing?.message);
});
