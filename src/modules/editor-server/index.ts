...
export { EditorServerConfig, EditorServerError, PeaksFile } from "./contracts.js";
export { computePeaks, PeakAccumulator, type PeaksIdentity, readPeaksCache, writePeaksCache } from "./peaks.js";
...
export function makeEditorServer(options: EditorServerOptions) {
  return Layer.unwrap(Effect.gen(function* () {
    ...
    const announce = Layer.effectDiscard(HttpServer.addressFormattedWith(address => Console.error(`Editor server for ${ctx.clip.bookId}/${ctx.clip.storyId} listening at ${address} (planning directory ${ctx.planningDirectory})`)));
