...
import { type AlignParameters, type AlignReport, type TimingEntries, WordTimingAuto, timingError, type TimingCode, WordTimingManual } from "./contracts.js";
export { alignRange, type AlignInput, measureRange, type MeasureInput, type SampleRange, type TimedWord } from "@animator/domain";
export { AlignParameters, AlignReport, AlignRun, TimingEntries, TimingEntry, TimingMeasure, WordTimingAuto, timingError, isTimingError, type TimingCode, WordTimingManual } from "./contracts.js";
...
type Code = TimingCode;
const fail = (code: Code, message: string) => Effect.fail(timingError({ code, message }));
const read = (path: string, limit: number) => readBounded(path, limit).pipe(Effect.mapError(e => timingError({ code: "IoFailed", message: e.message })));
const write = (path: string, bytes: Uint8Array) => writeAtomic(path, bytes).pipe(Effect.mapError(e => timingError({ code: "IoFailed", message: e.message })));
...
const decode = <S extends Schema.Top>(schema: S, bytes: Uint8Array, path: string) => decodeJson(schema, bytes, path, true).pipe(Effect.mapError(e => timingError({ code: "InvalidTiming", message: e.message })));
...
function loadOverlay<S extends Schema.Top & { readonly Type: { readonly clip: ClipIdentity; readonly words: TimingEntries } }>(ctx: OverlayContext, schema: S, path: string) {
  return Effect.gen(function* () {
    ...
    if (!(yield* fs.exists(path).pipe(Effect.mapError(() => timingError({ code: "IoFailed", message: `Cannot inspect ${path}.` }))))) return undefined;
...
export function writeManual(ctx: OverlayContext, words: TimingEntries) {
  return Effect.gen(function* () {
    ...
    const manual = yield* decode(WordTimingManual, bytes, "the supplied word timing").pipe(Effect.mapError(e => timingError({ code: "InvalidRequest", message: e.message })));
