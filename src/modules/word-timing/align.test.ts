...
import { alignRange, effectiveTiming, measureRange, type TimedWord, validateEntries } from "./index.js";
const RATE = 1000;
const word = (id: string, startSample: number, endSample: number): TimedWord => ({ id, startSample, endSample });
...
test("align: only words whose original start lies in the range are touched, regions are clipped to the range, and nothing ever leaves it", () => {
  ...
  for (const e of Object.values(entries)) assert.ok(e.startSample >= 1000 && e.endSample <= 2000, JSON.stringify(e));
...
test("align: heavy compression keeps every word at least one sample wide and inside its region", () => {
  ...
  for (let i = 1; i < spans.length; i++) assert.ok(spans[i]!.startSample >= spans[i - 1]!.startSample, "order is preserved");
});
