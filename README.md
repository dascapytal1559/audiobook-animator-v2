...
| `data/stories/<id>/` | One story: `story.json`, `audio/`, `transcript.json`, `transcript.txt`, `shots/<ulid>/record.json` with images, `decisions.json`, `story-map.json`, `scene-descriptions.json`, `word-timing.auto.json`, `word-timing.json`, `cache/` (regenerable peaks and speech regions), `transcription/` and `archive/` where a GPT transcript was made, and agent-written `*.md` documents. |
...
| `story-map.json` | agents and scripts | The story map (A62): subjects and sections as word-id ranges, pinned to the clip; reference images it names live inside the story directory. Read by the Scenes and Cast & world sections and `story map`. |
| `scene-descriptions.json` | writers, through `POST .../scene-descriptions` on the editor server | Description takes of declared shots, each under the shot's anchor word (A63, A65), pinned to the clip. Append-only: each take keeps its id, model, text, and time; a repeat is refused. Read by the Scenes section. |
...
pnpm --silent run cli story map --story the-great-silence             # story-map.json checked against the clip and transcript, resolved onto effective timing
...
pnpm --silent run cli timeline add --story the-great-silence --track claude-draft --at-seconds 12.5 --mode source-screenshot --label "Frame 042" --image path/to/frame.jpg
...
**Timeline.** A generation record is `shots/<ulid>/record.json` with its image beside it: the clip identity, an integer `startSample` on the clip clock, a mode, optional label, prompt, image, and notes, `createdAt`, and the producer. `decisions.json` holds per-shot overrides (`startSample`, `anchorWordId`, `mode`, `selected`, `hidden`, `notes`) and story settings such as the 16:9 frame. The merge (`mergeTimeline` in the domain package) applies overrides over records, groups shots with the same effective start within each image track, selects one per group (the explicitly selected, else the newest, never a hidden one), and stitches each track's selected shots so each holds until that track's next start, with an explicit opening gap; a decision that breaks a rule is refused naming the file. An anchored shot follows its word's effective start. New shots are prepared in hidden staging directories and published by one rename, so readers never encounter a half-written shot; a failed creation removes its staging directory. The one-time seed import (`animator timeline seed`) put the 14 treatment spans and 15 prototype panels into The Great Silence with deterministic ids, so a re-run is a no-op.

**Image tracks.** Each draft is an independent image track over the same narration. A record's optional `trackId` names it; existing records belong to `main`. Candidate selection, image holds, gaps, and merge-next operations stay within that track. The editor draws each track in its own lane; click a lane or use **Image track** to switch the preview. Links such as `http://localhost:5173/?story=the-great-silence&track=claude-draft&at=12.5` open a track at a clip time in seconds. `timeline add --track <id>` and the upload API's `trackId` field preserve draft membership; `source-screenshot` marks extracted movie frames.

**Story map, Scenes, and Cast & world.** `story-map.json` (A62) names a story's *subjects* (characters, locations, objects, motifs, each with a stable id, an optional description, reference images kept in the story directory, and *mentions*) and its *sections* (acts, chapters, scenes, beats). Every mention and section is an inclusive range of transcript word ids; the server checks the ids against the transcript and that sections nest or stay disjoint, and both the editor and the CLI resolve ranges onto effective word timing, so timing edits never invalidate a map. The editor shows it in two collapsible sections of the one page (A63), between the video and the transport. **Scenes** has the structure on the left (acts collapsed by default, the current chain highlighted and opened, each beat able to unfold its transcript passage with click-to-seek words) and one scene's detail on the right: heading, summary, subjects, its shots, and its transcript passage; clicking a section's title seeks the playhead and pins the detail, which otherwise follows the playhead. The map only navigates; it declares no shot. A *declared shot* (A65) is a word anchor on the visual timeline: a shot anchored to the word where the user says a new shot begins. The detail lists the declared shot holding at the scene's start and each one starting inside it, in timeline order, each headed by its first words and showing what each source produced for it, side by side and none picked: the image each image track starts with it, labelled with the track and opening large on click, and every description recorded for its anchor word in `scene-descriptions.json`, labelled with the model that wrote it, full text in the pane. Whatever else a track shows at the scene's start is listed once as no declared shot. A writer records a description take with `POST /api/stories/<id>/scene-descriptions` and a JSON body of `anchorWordId`, `model`, `text`, and optional `prompt` and `notes`; the server refuses a word no declared shot starts at and a repeat of the same shot, model, and text, and appends the take with an id, the time, and its producer. The editor generates and picks nothing. **Cast & world** has the subjects grouped by kind with images on the left and the selected subject's detail on the right; clicking a mention or an "appears in" chip seeks the playhead. The browser does not edit the map. Schema and rules are `packages/domain/src/story-map.ts`; the reader is `src/modules/story-map/`.
...
`packages/editor` is a Vite + React client that opens one verified story at a time. The story rides in the URL as `?story=<id>`; a bare URL opens the story this browser last showed, else the picker. It is one page of stacked sections (A63): a collapsible Video section with the letterboxed preview at the playhead, collapsible Scenes and Cast & world sections, a transport with clip and book clocks, then the lanes: the waveform with speech regions shaded beneath, up to three timing rows (Original, Auto, Edited), and the shot lane. Which sections are minimized, and how tall each is, is remembered per browser. Decisions and word-timing edits autosave 300 ms after each change; the working region is client-only.
...
**Preview subtitles.** The **Subtitles** button below the picture starts on; **Highlight word** starts off. Both choices are remembered in this browser across stories. Subtitles keep the working transcript's wording and punctuation, displaying a whole sentence in at most two lines or automatically dividing a long sentence into successive phrases. They use effective word timing, follow pending manual edits, remain visible through gaps inside a phrase, and clear 250 ms after the last word or at the next subtitle's start. Pausing and seeking update the same display. Highlighting follows the transcript's existing timed tokens, so a provider token containing several words highlights together. Text scales with the picture frame; resizing or changing a shot does not regroup it. Subtitle sentences are distinct from the timeline's pause-based chunks. Manual subtitle editing and movie export come later; no subtitle files are written. The shared rules and defaults are in `packages/domain/src/subtitles/` ([A61](docs/decisions/A61-derived-preview-subtitles.md)).

Word timing in the Edited row: click selects a word (or a whole sentence in Sentences mode), Shift-click extends the selection to a contiguous range, Escape clears it. Dragging any selected box moves every selected word by the same amount with durations preserved; the leading edge snaps to speech-region onsets within 8 px (Alt disables), the move is clamped so the group never crosses its unselected neighbours or the clip edges, and the toolbar shows the delta while dragging. Comma and period nudge the selection by 10 ms. Cmd/Ctrl+Z undoes and Shift+Cmd/Ctrl+Z redoes timing edits (client-side stack, each step autosaves). "Align selection" runs the server's automatic pass over the selected span and shows before → after median, p10, p90 boundary error and inside-speech fraction in the toolbar; there is no whole-clip button in the UI. A shot dropped on a word or sentence start is anchored to that word (`anchorWordId` in its decision) and follows the word's effective start; the marker carries an anchor glyph. Dropping a marker anywhere else detaches it.
...
| `GET …/map` | `story-map.json` as written (A62), strictly decoded, pinned to the clip (409 otherwise), checked against the transcript's word order and nesting rule (500 `InvalidMap` naming the rule), each subject image given a `url`. 404 `NotFound` when the file does not exist. Reread on every request. |
| `GET …/map/subjects/:subjectId/images/:index` | One reference image the map names, resolved inside the story directory; 404 otherwise. |
