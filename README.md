...
| `story-map.json` | agents and scripts | The story map (A62): subjects and sections as word-id ranges, pinned to the clip; reference images it names live inside the story directory. Read by the World map section and `story map`. |
...
**Story map and explorer.** `story-map.json` (A62) names a story's *subjects* (characters, locations, objects, motifs, each with a stable id, an optional description, reference images kept in the story directory, and *mentions*) and its *sections* (acts, chapters, scenes, beats). Every mention and section is an inclusive range of transcript word ids; the server checks the ids against the transcript and that sections nest or stay disjoint, and both the editor and the CLI resolve ranges onto effective word timing, so timing edits never invalidate a map. The editor's **World map** section (A63; a collapsible section of the one editor page, between the video and the transport) shows the structure on the left (acts collapsed by default, the current chain highlighted and opened, each beat able to unfold its transcript passage with click-to-seek words) and the cast with images on the right; clicking a section, a mention, or an "appears in" chip seeks the playhead. The browser does not edit the map. Schema and rules are `packages/domain/src/story-map.ts`; the reader is `src/modules/story-map/`.
...
`packages/editor` is a Vite + React client that opens one verified story at a time. The story rides in the URL as `?story=<id>`; a bare URL opens the story this browser last showed, else the picker. It is one page of stacked sections (A63): a collapsible Video section with the letterboxed preview at the playhead, a collapsible World map section, a transport with clip and book clocks, then the lanes: the waveform with speech regions shaded beneath, up to three timing rows (Original, Auto, Edited), and the shot lane. Which sections are minimized is remembered per browser. Decisions and word-timing edits autosave 300 ms after each change; the working region is client-only.
