...
| `scene-descriptions.json` | writers, through `POST .../scene-descriptions` on the editor server | Description takes per map section (A63), pinned to the clip. Append-only: each take keeps its id, model, text, words, and time; a repeat is refused. Read by the Scenes section. |
...
**Story map, Scenes, and Cast & world.** `story-map.json` (A62) names a story's *subjects* (characters, locations, objects, motifs, each with a stable id, an optional description, reference images kept in the story directory, and *mentions*) and its *sections* (acts, chapters, scenes, beats). Every mention and section is an inclusive range of transcript word ids; the server checks the ids against the transcript and that sections nest or stay disjoint, and both the editor and the CLI resolve ranges onto effective word timing, so timing edits never invalidate a map. The editor shows it in two collapsible sections of the one page (A63), between the video and the transport. **Scenes** has the structure on the left (acts collapsed by default, the current chain highlighted and opened, each beat able to unfold its transcript passage with click-to-seek words) and one scene's detail on the right: heading, summary, subjects, its takes, and its transcript passage; clicking a section's title seeks the playhead and pins the detail, which otherwise follows the playhead. The takes are what each source produced for the scene, side by side and none picked: one image per image track, the shot that track holds at the scene's start by the preview's own rule, labelled with the track and opening large on click; and every description recorded for the section in `scene-descriptions.json`, labelled with the model that wrote it, full text in the pane. A writer records a description take with `POST /api/stories/<id>/scene-descriptions` and a JSON body of `sectionId`, `model`, `text`, and optional `prompt` and `notes`; the server appends it with an id, the section's words, the time, and its producer, refusing a repeat of the same section, model, and text. The editor generates and picks nothing. **Cast & world** has the subjects grouped by kind with images on the left and the selected subject's detail on the right; clicking a mention or an "appears in" chip seeks the playhead. The browser does not edit the map. Schema and rules are `packages/domain/src/story-map.ts`; the reader is `src/modules/story-map/`.
