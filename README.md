...
**Story map, Scenes, and Cast & world.** `story-map.json` (A62) names a story's *subjects* (characters, locations, objects, motifs, each with a stable id, an optional description, reference images kept in the story directory, and *mentions*) and its *sections* (acts, chapters, scenes, beats). Every mention and section is an inclusive range of transcript word ids; the server checks the ids against the transcript and that sections nest or stay disjoint, and both the editor and the CLI resolve ranges onto effective word timing, so timing edits never invalidate a map. The editor shows it in two collapsible sections of the one page (A63), between the video and the transport. **Scenes** has the structure on the left (acts collapsed by default, the current chain highlighted and opened, each beat able to unfold its transcript passage with click-to-seek words) and one scene's detail on the right: heading, summary, subjects, the places for its description and image (labelled not generated yet until a later decision fills them), and its transcript passage; clicking a section's title seeks the playhead and pins the detail, which otherwise follows the playhead. **Cast & world** has the subjects grouped by kind with images on the left and the selected subject's detail on the right; clicking a mention or an "appears in" chip seeks the playhead. The browser does not edit the map. Schema and rules are `packages/domain/src/story-map.ts`; the reader is `src/modules/story-map/`.
